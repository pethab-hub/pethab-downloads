# PetHab Downloads

Public download repository for PetHab character packages.

## Download

Open [characters](./characters/) and download the `download.zip` file for the character you want to use.

Each ZIP contains the files needed to install and display the character locally, including `pet.json` and the character spritesheet. Entries marked as a bundle also contain the listed optional assets.

## Characters

- [Afterhours Signal Host](./characters/afterhours-signal-host/download.zip)
- [AI Ops Foxcat](./characters/ai-ops-foxcat/download.zip)
- [Battery Dock Nurse](./characters/battery-dock-nurse/download.zip)
- [Blade Command Muse](./characters/blade-command-muse/download.zip)
- [Branch Knot Tailor](./characters/branch-knot-tailor/download.zip)
- [Build Crank Runner](./characters/build-crank-runner/download.zip)
- [Cable Cat Nester](./characters/cable-cat-nester/download.zip)
- [Cable Cat Nester G5](./characters/cable-cat-nester-g5/download.zip)
- [Cloud Archive Clerk](./characters/cloud-archive-clerk/download.zip)
- [Hub Crab Mechanic](./characters/hub-crab-mechanic/download.zip)
- [Hubframe Shifter](./characters/hubframe-shifter/download.zip)
- [Luma Sprint Scout](./characters/luma-sprint-scout/download.zip) — Pet + PC backgrounds (1920×1080, 2560×1440, 3840×2160)
- [Mail Courier Muse](./characters/mail-courier-muse/download.zip)
- [Mic Pop Guard Pup](./characters/mic-pop-guard-pup/download.zip)
- [Namecard Sleeve Collector](./characters/namecard-sleeve-collector/download.zip)
- [Office Orbit Sister](./characters/office-orbit-sister/download.zip)
- [Patch Review Smith](./characters/patch-review-smith/download.zip)
- [Print Queue Postmaster](./characters/print-queue-postmaster/download.zip)
- [Screen Arm Mini-Rigger](./characters/screen-arm-mini-rigger/download.zip)
- [Sheet Formula Cadet](./characters/sheet-formula-cadet/download.zip)
- [Silk Formula Analyst](./characters/silk-formula-analyst/download.zip)
- [Silver Aegis Warrior](./characters/silver-aegis-warrior/download.zip)
- [Suncrest Hero](./characters/suncrest-hero/download.zip) — Pet + PC background (1920×1080)
- [Tab Scout Runner](./characters/tab-scout-runner/download.zip)
- [Time Slot Keeper](./characters/time-slot-keeper/download.zip)
- [Trackball Homebody](./characters/trackball-homebody/download.zip)
- [Velvet File Concierge](./characters/velvet-file-concierge/download.zip)
- [Voice Memo Shell Listener](./characters/voice-memo-shell-listener/download.zip)
- [White Sprint Runner](./characters/white-sprint-runner/download.zip)
- [Window Operator Muse](./characters/window-operator-muse/download.zip)

## Use

1. Download a character ZIP.
2. Extract the ZIP locally.
3. Add the extracted character folder to your local PETS folder.
4. Restart or refresh the host app and select the character.

Bundled PC background setup:

- Luma Sprint Scout: open `wallpapers/README.md` and manually select the image matching your display.
- Suncrest Hero: open `bonus/README-ja.md` and use `bonus/wallpapers/suncrest-hero-desktop-wallpaper.png`.

Wallpaper setup is not automatic.

The packages are independent custom character assets. Redistribution, resale as-is, or claims of official endorsement are not included.
